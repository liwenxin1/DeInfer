...
if __name__=="__main__":
    ...
    parser.add_argument("--interference_num",type=int,default=4)
    ...
    if mps_set!=0:
        ...
        interference_num=args.interference_num
        pool=mp.Pool(processes=interference_num)
        mps_set=int(mps_set/interference_num)
        ...
        for i in range(interference_num):
            pool.apply_async(os.system,args=(commend,))
    ...
    #测试代码
    time.sleep(2)#给干扰进程一定的启动时间
    m.set_input("data",tvm.nd.array((np.random.uniform(size=shape)).astype("float32")))
    
    value=m.benchmark(dev,repeat=5,min_repeat_ms=3000).mean
