...
#这一部分提供算法模拟，不进行真正的推理
#system3部分假设数据到达满足泊松分布，并且先设定模型的到达率进行分析
...
class Scheduler_Simulator:
    ...
    def data_inference(self,data,time_out,GID,TRP_ID,threshold):
        self.GPU_info[GID][TRP_ID].threshold=threshold+self.GPU_info[GID][TRP_ID].\
            inference_time*math.ceil(len(data)/self.GPU_info[GID][TRP_ID].batchsize)
    
    def get_TRP_data_length(self,GID,TRP_ID):
        
        return np.max([math.ceil((self.GPU_info[GID][TRP_ID].threshold-time.time())/self.GPU_info[GID][TRP_ID].inference_time),0])
...
class Time_Slice_System:
    def __init__(self,GPU_number:int,modelRepository,arrivalrate:dict) -> None:
        ...
        #记录模型的到达强度，用于采集数据，没有其他用途
        self.model_data_arrival=0
        #用于记录队列的最大长度，用于采集数据，没有其他用途
        self.deal_queue=0
        self.queue_limit=0
        
       
    #该函数只使用在了数据均匀到达的情况，无需修改该函数
    ...
    def get_TRP_data_queue_length(self,GID:int,TRP_ID:int):
        return self.scheduler.get_TRP_data_length(GID,TRP_ID)
    #设置模型数据的到达强度
    def model_arrivalrate(self,modelname:str,arrivalrate:int):
        self.arrivalrate[modelname]=arrivalrate
        
    #获取满足SLO的batchsize最大，mps最小的资源参数
    #目前假设一个任务只存在一个模型进行服务，则模型到达强度=数据到达强度
    #根据M/D/1的排队论模型
    #D=(arrivalxslo+2-sqrt(arrival^2xslo^2+4))/2xarrival
    def search_params(self,modelname,split_factor,slo):
        ...
        D=(self.arrivalrate[modelname]*slo+2-math.sqrt(self.arrivalrate[modelname]**2*slo**2+4))/(2*self.arrivalrate[modelname])
        while(mps_set<=100):
            ...
            for batchsize in batchsize_support_list[::-1]:
                ...
                temp_time=runtime/batchsize
                if runtime<=slo/2 and temp_time<=D:
                    #计算理想队列最大长度
                    p=self.arrivalrate[modelname]*temp_time
                    self.queue_limit=slo/temp_time-1
                    MD1_quque=p**2/(1-p)
                    self.deal_queue=min(self.queue_limit,MD1_quque)
                    print(MD1_quque)
            ...
            for modelname in self.MI.model_info_dict.keys():
                ...
                if self.model_queue.qsize(modelname)!=0:
                    ...
                    for GID,TRP_ID,batch_size,mps_set in model_list:
                        ...
                        #     mps=mps_set)
                        inference_time=self.scheduler.GPU_info[GID][TRP_ID].inference_time
                        ...
                        if i_interval!=None:
                            ...
                            #print("TRP_ID{} modelname:{},data_length:{},finish data:{},mps_set:{}".format(TRP_ID,modelname,data_length,i_interval[1]-i_interval[0],mps_set))
                            #将数据交给进程进行推理
                            
                            self.scheduler.data_inference(infer_data,infer_time_out,GID,TRP_ID,model_threshold)
...
#数据每秒到达呈现泊松分布
#通过这种方式得到的实际数据流分布小于指定的泊松强度
def producer(tts:Time_Slice_System,modelname:str):
    #设置动态变化的到达率
    arrivalrate=tts.arrivalrate[modelname]
    np.random.seed(0)
    
    while tts.running:
        #------------------------------
        #这部分代码可以设置到达率的变化
        data_num=np.random.poisson(arrivalrate)
        #------------------------------
        tts.model_data_arrival=data_num
        ...
        for i in range(data_num):
            ...
            #0.0025平衡掉时间差 0.00075同理
            time.sleep(1/data_num-0.00075)
...
#记录每一秒的数据情况
def record(tts:Time_Slice_System,record_time:int,resource_list,arrivalrate_list,queue_length_list):
    """_summary_
    ...
    for _ in range(record_time):
        ...
        #数据的到达强度
        arrivalrate_list.append(tts.model_data_arrival)
        #队列长度
        queue_length_list.append(tts.get_TRP_data_queue_length(0,0))
       
        time.sleep(1)
    tts.running=False
    
def draw_with_change_arrivalrate(modelname,slo,producer_func,record_time,save_file,arrivalrate:dict):
        slo_dict={modelname:slo}
        tss=Time_Slice_System(1,"modelDeploy/modelRepository",arrivalrate=arrivalrate)
        #数据到达函数
        ...
        thread=threading.Thread(target=record,args=(tss,record_time,resource_list,arrivalrate_list,queue_length_list))
        thread.start()
        ...
        #创建第三个子图
        deal_queue=tss.deal_queue
        queue_limit=tss.queue_limit
        ax3 = fig.add_subplot(3, 1, 3)
        print(np.mean(queue_length_list))
        ax3.plot(time_list, queue_length_list)
        ax3.plot(time_list,[deal_queue for _ in range(len(queue_length_list))],"r--")
        ax3.plot(time_list,[queue_limit for _ in range(len(queue_length_list))],"g--")
        ax3.set_title('{} queue length'.format(modelname))
        ...
        #保存数据
        np.save("modelDeploy/data_deal/{}_system3_data.npy".format(modelname),np.array([resource_list,arrivalrate_list,queue_length_list,[tss.deal_queue,tss.queue_limit]]))
...
if __name__=="__main__":
    ...
    parser.add_argument("--arrival_rate",type=int,default=100)
    ...
    draw_with_change_arrivalrate(modelname,slo,producer,60,"modelDeploy/pic/_{}_slo2_arr300_.png".format(modelname),arrivalrate)
    os._exit(0)
